...
class FeaturesExtractor:
    ...
    def __get_sentences(self, raw_text: str) -> list[str]:
        ...
        return [
            s.text for s in sentenize(raw_text) if not self.__URL_PAT.search(s.text)
        ]
    ...
    def __sents_to_features_vec(self, text_sents: list[str]) -> list[list[str]]:
        ...
        have_quote = [re.search(self.__QUOTE_PAT, s) is not None for s in text_sents]

        sents_count = len(text_sents)
        sents_pos = [i / sents_count for i in range(1, sents_count + 1)]

        text_len = sum([len(s) for s in text_sents])
        sents_len = [len(s) / text_len for s in text_sents]
    ...
    def convert_text_to_df(self, text: str) -> pd.DataFrame:
        ...
        features_df = pd.DataFrame(
            features, columns=["have_quote", "sent_len", "sent_pos"]
        )
