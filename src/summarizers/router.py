...
summarizers_router = APIRouter(
    prefix="/summarizers", tags=["Модели для реферирования 👾"], dependencies=[AuthDep]
)


@summarizers_router.get(
    "", summary="Получить список доступных моделей", status_code=status.HTTP_200_OK
)
...
@summarizers_router.put(
    "/selected", summary="Выбрать модель", status_code=status.HTTP_200_OK
)
def set_model(model_name: Annotated[str, Query(description="Название модели")]) -> str:
